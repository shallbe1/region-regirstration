...
if 1:
    if 1:
        conf_file='superpoint+lightglue_homography.yaml'
        set_seed(0)
        model_path = root / "gluefactory" / "configs" / 'superpoint+lightglue_homography.yaml'
        ...
        if 1:#self.visualize:
            pred = map_tensor(
                pred, lambda t: t.cpu().detach().numpy() if isinstance(t, Tensor) else t
            )
