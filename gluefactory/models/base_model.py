...
class BaseModel(nn.Module, metaclass=MetaModel):
    ...
    def __init__(self, conf):
        ...
        self._init(conf)
        #print(conf.trainable,"++++++++__________+++++++++")
        #print(conf)
        #if conf.trainable:
        #    print(None-1)
        ...
        #print(conf.name)
        #if str(conf.name)=="two_view_pipeline":
        #    print(conf.name,"==========!!!!!!!!!!",None-1)
        if not conf.trainable:
            for  p in self.parameters():
            #for n, p in self.named_parameters():
            #    if 'extractor' in n:
                #if 'convD' in n or "convP" in n:
            #        p.requires_grad = True
                    #print(n,"~~~~~~~~~~~")
                #print(p,"0000000000000000000000000000000=========")
            #    else:
