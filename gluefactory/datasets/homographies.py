...
class HomographyDataset(BaseDataset):
    default_conf = {
        ...
        "image_list": "our.txt",  # optional: list or filename of list
...
class _Dataset(torch.utils.data.Dataset):
    ...
    def getitem(self, idx):
        ...
        if random_num>0.5:
        #if name.split("/")[-2]=="RGB":
            #print(name,"----",self.image_dir /"IR"/name.split("/")[-1])
            img_r=read_image(self.image_dir /"IR"/name.split("/")[-1], False)
        else:
        #elif name.split("/")[-2]=="IR":
            img_r=read_image(self.image_dir /"RGB"/name.split("/")[-1], False)
        #else:
        #    print(name.split("/")[-2],"-----------")
        ...
        #print("patch_shape",ps,size,img_r.shape[:2][::-1],name,"----",random_num)
        ...
        data1 = self._read_view(img_r, self.conf.homography, ps, left=False)
